...
export default function Resumo() {
  ...
  const isCurrentMonthView = selectedMonth === realCurrentMonth && selectedYear === realCurrentYear;
  const isPastMonthView = selectedYear < realCurrentYear || (selectedYear === realCurrentYear && selectedMonth < realCurrentMonth);
  const isFutureMonthView = selectedYear > realCurrentYear || (selectedYear === realCurrentYear && selectedMonth > realCurrentMonth);

  const renderDayCards = (days: any[], emptyMessage: string, useGrid: boolean = false) => {
    if (days.length === 0) {
      return (
    ...
    return (
      <div className={useGrid ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" : "flex flex-col gap-4"}>
    ...
    <Layout activePath="/resumo" hideFooterOnMobile={true} hideSearch={true}>
      <div className="p-4 lg:p-8 max-w-7xl mx-auto flex flex-col h-full overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <div className={`grid grid-cols-1 ${isCurrentMonthView ? 'lg:grid-cols-3' : ''} gap-6 items-start`}>
            ...
            {(isCurrentMonthView || isPastMonthView) && (
              <div className={`flex flex-col gap-4 ${isCurrentMonthView ? 'lg:col-span-1 order-3 lg:order-1' : ''}`}>
                <div className="bg-surface border border-outline-variant/20 rounded-2xl p-6 shadow-sm">
                  ...
                  {renderDayCards(data.pastDays, 'Sem registros', isPastMonthView)}
                </div>
              </div>
            )}
            ...
            {(isCurrentMonthView || isFutureMonthView) && (
              <div className={`flex flex-col gap-4 ${isCurrentMonthView ? 'lg:col-span-1 order-2 lg:order-3' : ''}`}>
                <div className="bg-surface border border-outline-variant/20 rounded-2xl p-6 shadow-sm">
                  ...
                  {renderDayCards(data.futureDays, 'Sem próximos registros', isFutureMonthView)}
                </div>
              </div>
            )}
